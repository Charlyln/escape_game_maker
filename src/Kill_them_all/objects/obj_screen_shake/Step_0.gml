if (instance_exists(obj_player)) {
	if (!obj_player.dead && !obj_end_game.final_ended) {
		if (instance_exists(prt_weapon)) {
			x_value = camera_get_view_x(view_camera[0]);
			y_value = camera_get_view_y(view_camera[0]);

			if (
				prt_weapon.success_surcharge &&
				prt_weapon.reloading &&
				prt_weapon.alarm[0] <= prt_weapon.reload_sprite_time
			) {
				shake = true;
			...
			if (shake) {
				...
				camera_set_view_pos(
					view_camera[0],
					x_value + random_range(-shake_range, shake_range),
					y_value + random_range(-shake_range, shake_range)
				);
				...
				camera_set_view_pos(
					...
					y_value +
						random_range(-bullet_shake_range, bullet_shake_range)
				);
